...
Write-Host "`n> $cmd`n"
if ($isSpec) {
    # Paths inside a .spec (Analysis(["main.py"]), pathex=["."]) resolve against
    # the current directory, not the spec's own — a spec written to be run from
    # its project folder fails with "script not found" from anywhere else.
    # distpath and workpath are absolute, so the output still lands in dist/.

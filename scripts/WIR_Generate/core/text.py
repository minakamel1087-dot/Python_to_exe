...
def tokenize(area: str) -> list[str]:
    ...
    return _expand_number_runs(tokens)
...
def match_key(token: str) -> str:
    ...
    if stem.startswith("APART") or stem.startswith("FLAT") or stem in _UNIT_WORDS:
...
def nearest(name: str, candidates: list[str]) -> str:
    """The closest known name, or "" when nothing is close enough. The cap
    keeps a genuinely new room from being "corrected" into an unrelated
    one."""
    probe = area_key(name)
    if not probe:
        return ""

    limit = 1 + len(probe) // 4          # roughly a quarter of the name
    best, best_distance = "", 99
    for candidate in candidates:
        distance = edit_distance(probe, area_key(candidate))
        if distance < best_distance:
            best, best_distance = candidate, distance

    return best if best_distance <= limit else ""

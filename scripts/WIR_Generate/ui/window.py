...
from PySide6.QtWidgets import (
    ...
    QMainWindow, QMessageBox, QPushButton, QTabWidget, QTextEdit,
    QVBoxLayout, QWidget,
)
...
ACTIONS = [
    ...
    ("generate", "Generate WIRs", "Cover, checklist, attachments, merged to one PDF", "generate"),
...
class Worker(QObject):
    ...
    def _dispatch(self, book: LiveWorkbook) -> list[RunResult]:
        ...
        if task == "fix_links":
            return [tasks.fix_links.run(book)]
        if task == "generate":
            return [tasks.generate.run(book, self.params.get("output", ""))]
        if task == "extract_pdfs":
            return [tasks.extract_pdfs.run(book, self.params.get("destination", ""))]
        if task == "import_data":
            return [tasks.import_register.run(book, self.params.get("source", ""))]
...
class MainWindow(QMainWindow):
    ...
    def _actions_tab(self) -> QWidget:
        ...
        self.email_box = QCheckBox("Draft the submission email in Outlook after Copy To History")
    ...
    def _section(self, key: str) -> QWidget:
        ...
        for section, title, description, task in ACTIONS:
            if section != key:
                continue
            card = ActionCard(title, description, section, filled=(task == "generate"))
    ...
    def _ask_for_paths(self, task: str) -> dict | None:
        ...
        if task == "import_data":
            ...
            return None if not path else {"source": os.path.normpath(path)}
        ...
        if task == "generate":
            folder = QFileDialog.getExistingDirectory(
                self, "Where should the generated WIRs go? (Cancel to use the default)"
            )
            return {"output": os.path.normpath(folder) if folder else ""}

        if task == "copy_history":
            return {"email": self.email_box.isChecked(), "greeting": ""}

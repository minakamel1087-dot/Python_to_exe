...
(Staircase 1, 2 and Lift Core)   -> Staircase 1, Staircase 2, Lift Core
...
their **local** paths into column AA, semicolon-separated.
...
```
python tools\deploy.py --final-path "C:\WIR Tools"
```
...
```
WIR Tools\
    WIR Tools.lnk         double-click to open the window
...
- **the same local path on every PC**, e.g. `C:\WIR Tools`.
...
And `WIR Tools.lnk` targets `pythonw.exe` rather than `python.exe`, so no
...
```
pyinstaller --noconfirm --clean "WIR Tools.spec"
"dist\WIR Tools.exe" --selftest
```
...
`WIR Tools.spec` lists these by hand and adds
...
- `--check` gives the headless output from a terminal, either as
  `WIR Tools.exe --check` or `runtime\python.exe app\main.py --check`.
- The target machine needs the Microsoft Visual C++ 2015–2022
  ...
  practice.
- `WIR Tools.spec` is checked in — do not let PyInstaller regenerate it.

...
def format_area(area: str, floor: str, ref: ReferenceData) -> str:
    ...
    for token in tokenize(area):
        ...
        if is_unit_code(key) and claimed:
            ...
            # Known spelling wins; otherwise take the closest known name
            # so the suggestion carries the correction itself rather than
            # asking a question. Nothing close enough stays as typed.
            canonical = (
                ref.canonical_area(token)
                or nearest(token, ref.area_names)
                or token
            )
...
def area_comment(area: str, floor: str, package: str, building: str,
    ...
    for token in tokenize(area):
        ...
        elif not nearest(token, ref.area_names):
            # A close match is already applied in the suggestion itself,
            # so saying "did you mean" as well is just noise. Only what
            # cannot be corrected needs a note.

...
def run(book: LiveWorkbook, ref: ReferenceData) -> RunResult:
    ...
    # Server -> column N as the register files it; Local -> column AD,
    # the extracted copy on this PC.
    mode = pathmap.read_mode(book)
    path_column = (config.Register.LOCAL_PATH if mode == pathmap.LOCAL
                   else config.Register.PATH)
    approved = _index_approved(register_rows, ref, path_column)
    ...
    source = "column AD, local" if mode == pathmap.LOCAL else "column N, server"
    result.line(f"Paths taken from:      {source}")
...
def _index_approved(rows: list[list], ref: ReferenceData,
                    path_column: int) -> dict[str, list[tuple[str, str]]]:
    ...
    for row in rows:
        ...
        index[key].append((
            ...
            # Column N on the share, or column AD locally, following the
            # Server/Local buttons. Whichever it is ends up in log column
            # AA, and Generate then takes the first page of those files -
            # so the choice made here decides what Generate has to reach.
            text_of(row[path_column - 1]),
        ))
    return index

...
def _index_approved(rows: list[list], ref: ReferenceData) -> dict[str, list[tuple[str, str]]]:
    ...
    for row in rows:
        ...
        index[key].append((
            ...
            # Column N, the path as the register stores it - on the share,
            # not the local extract folder in AD. Changed 2026-08-17 on
            # instruction: these are the commented WIRs as filed, and
            # Generate now takes only their first page anyway.
            #
            # The consequence is that both this task and Generate need the
            # server connected; AD did not.
            text_of(row[config.Register.PATH - 1]),
        ))
    return index

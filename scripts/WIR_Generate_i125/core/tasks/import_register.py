...
def run(book: LiveWorkbook, source_path: str,
    ...
    # Read before the register is opened - it comes from this workbook,
    # and column AD is derived from it row by row.
    table = pathmap.load(book)
    if not table.pairs:
        result.line(f"No '{config.PATHS_SHEET}' sheet: column AD will repeat "
                    "the server paths from column N.")
    ...
    try:
        ...
        for name in SOURCE_SHEETS:
            ...
            derived = [_derive(row, table) for row in rows]
            target.Range(
    ...
    fix_prefix.apply(book, result, table)
...
def _derive(row: list, table: pathmap.PathMap) -> list:
    """AA = number and status, AB = location code, AC = area,
    AD = the local path the commented PDF actually opens from.

    AD comes from substituting the server prefix in column N for its
    local twin. It used to be built from the last 52 characters of the
    path, which worked only because the WIR file names happen to be
    exactly that long - any other length silently produced a wrong path."""
    ...
    local_path, _ = table.to_local(path)

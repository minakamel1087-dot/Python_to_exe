...
from ..paths import (
    exists, is_network_path, repair, share_available, split_paths, trim_slashes,
    unquote,
)
...
class _Run:
    ...
    def check_attachments(self, row: int, wir_no: str, values: list) -> None:
        for column, label, warn in ATTACHMENT_COLUMNS:
            ...
            # Off the network, every share path fails to resolve. Counting
            # those as missing attachments would bury the real problems.
            if not self.share_ok and is_network_path(raw):
                continue

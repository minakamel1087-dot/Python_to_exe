"""
Builds the portable "WIR Tools" folder.
...
python.exe, which is signed by the Python Software Foundation.
    ...
    WIR Tools\\
        WIR Tools.lnk         double-click this
...
shortcuts are written for it:
    ...
    or the same local path on every PC, such as C:\\WIR Tools.
...
The full manual is "WIR Tools - User Manual.docx".
"""
...
def build(runtime_source: str, output: str, final: str) -> int:
    ...
    manual = os.path.join(PROJECT, "WIR Tools - User Manual.docx")
    ...
    _make_shortcut(
        os.path.join(output, "WIR Tools.lnk"),
    ...
    _make_shortcut(
        ...
        "Prove this machine can run WIR Tools",
        final,
    )
    print("  shortcuts: 2")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the portable WIR Tools folder.",
    ...
    parser.add_argument("--output", default=os.path.join(PROJECT, "dist", "WIR Tools"),
